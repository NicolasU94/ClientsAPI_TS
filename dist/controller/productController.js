...
const productController = {
    ...
    searchProducts: async (req, res, next) => {
        try {
            ...
            const product = await Products_1.default.find({ name: new RegExp(query, "i") });
            res.json(product);
        }
        catch (error) {
            res.status(500).send(error);
    ...
    updateProductById: async (req, res) => {
        try {
            let prevProduct = await Products_1.default.findById(req.params.id);
            ...
            const product = await Products_1.default.findOneAndUpdate({ _id: req.params.id }, newProduct, { new: true });
            res.json(product);
        }
        catch (error) {
            res.status(500).send(error);
    ...
    deleteProductById: async (req, res) => {
        try {
            const result = await Products_1.default.findOneAndDelete({ _id: req.params.id });
            if (!result) {
                res.status(404).json({ message: "Product Not found" });
            }
            else {
